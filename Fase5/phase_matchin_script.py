...
def compute_k_l(path):
    ...
    return kp, ks, ki, lamp, lams, tau_s, tau_i
...
def compute(path):
    ...
    # Phase mismatch
    kp_TE, ks_TE, ki_TE, lamp, lams, tau_s, tau_i = compute_k_l(pathTE)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.dirname(path) + '/corr' + os.path.basename(path) + '.png')
